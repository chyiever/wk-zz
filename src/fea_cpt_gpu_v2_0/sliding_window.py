...
def _compute_one_stft_chunk(
    ...
    """Compute GPU STFT for one window chunk and return it in shared memory."""
    widest_params = None
    widest_high = 0.0
    for params in params_map.values():
        if params.main_band_hz[1] > widest_high:
...
def _iter_adaptive_stft_chunks(
    ...
    """Yield STFT chunks, shrinking the batch if GPU memory is tight."""
    ...
    while chunk_start < len(windows):
        ...
        try:
            ...
            if batch_size <= min_batch_size or not _is_cuda_oom(exc):
                raise
...
def _preload_file(
    ...
    try:
        ...
        logging.getLogger('fea_cpt_gpu_v2_0').warning('预加载文件失败 %s: %s', file_path, e)
        return None
...
def build_sliding_window_dataset(
    ...
    if config.enable_numa_binding:
        bound = _bind_numa()
        if bound:
            logging.getLogger('fea_cpt_gpu_v2_0').info('NUMA 绑定成功')
    ...
    logger = logging.getLogger('fea_cpt_gpu_v2_0')
