"""滑窗批量特征提取模块（v2.1：共享STFT + NUMA绑定 + 大批次）。

用于对连续真实数据（npz/tdms）进行滑窗全量特征提取。
支持多文件夹批量处理、200kHz/500kHz 统一采样率、窗级并行。

v2.1 优化（对比 v2）：
1. 跨频带共享 STFT：先对宽频带做一次 STFT，各子带从结果中切片，
   避免每个频带独立重复做 STFT，STFT 计算量降低 5/6
2. NUMA 绑定：psutil 绑定 CPU 亲和性，减少跨 NUMA 内存访问
3. 大批次：WINDOW_BATCH_SIZE 从 256 提升到 2048，
   减少进程池创建/销毁开销
"""
...
def _auto_detect_workers() -> int:
    """自动检测 CPU 核心数，推荐作为默认 workers 数量。"""
    cpu_count = os.cpu_count() or 4
    return max(1, cpu_count - 2)


def _bind_numa() -> bool:
    """绑定当前进程到所有可用 CPU 核心（NUMA 亲和性优化）。

    Returns:
        True 表示绑定成功，False 表示跳过
    """
    if psutil is None:
        return False
    try:
...
def compute_shared_stft(
    ...
    """对窗口信号计算一次宽频带 STFT，各子带从结果中切片。

    核心思路：
    - 用最宽频带的 STFT 参数做一次计算
    - 较窄频带直接从 STFT 结果中按频率索引切片
    - 保证特征计算公式不变，仅避免重复 STFT 调用

    Returns:
        {band_name: {'stft_freqs', 'stft_times', 'stft_complex', 'stft_power',
                     'short_freqs', 'short_times', 'short_power'}}
    """
    # 找最宽频带（决定 STFT 的频率范围）
    widest_params = None
    widest_high = 0.0
...
def compute_all_features_for_window(
    ...
    Args:
        ...
        shared_stft: 共享 STFT 结果（v2.1 优化），None 则各频带独立计算
    ...
    if shared_stft is not None:
        ...
        for band_name, params in params_map.items():
            ...
            # 滤波（仍需按频带滤波，因 build_context 内部会用滤波后信号）
            ...
            # 使用全频带的 complex 做掩码和 ISTFT（保证特征公式一致）
        ...
        # 原始路径：各频带独立调用 build_context + compute_all_features
        rec = FeatureRecord(
            sample_id='w', sample_name='w', sample_type='raw',
            sample_type_code=0, path=Path('.'),
            signal=np.asarray(window_signal, dtype=float),
...
def process_source_file(
    ...
    for b0 in range(0, len(windows), config.window_batch_size):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            futures = [
                ex.submit(
                    _process_one_window, w, signal_pre, effective_rate, params_map,
                    base_meta, start_dt,
                    config.enable_shared_stft, config.bands,
                )
                for w in chunk
            ]
...
def build_sliding_window_dataset(
    ...
    if config.enable_numa_binding:
        bound = _bind_numa()
        if bound:
            logging.getLogger('fea_cpt_gpu_v2_1').info('NUMA 绑定成功')
