...
def compute_all_features(context: FeatureContext) -> FeatureResult:
    ...
    hankel_cols = max(4, min(len(residual) // 3, 128))
    hankel_rows = len(residual) - hankel_cols + 1
    if hankel_rows >= 4:
        hankel = np.column_stack([residual[i : i + hankel_rows] for i in range(hankel_cols)])
        u, s, vh = np.linalg.svd(hankel, full_matrices=False)
        rank = min(2, len(s))
        hankel_hat = (u[:, :rank] * s[:rank]) @ vh[:rank, :]
        features["Q_MP"] = float(1.0 - np.linalg.norm(hankel - hankel_hat) / (np.linalg.norm(hankel) + eps))
    else:
        features["Q_MP"] = 0.0
