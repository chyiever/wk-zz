"""Shared preprocessing and signal-analysis utilities (v2: GPU-accelerated STFT).

v2 优化：使用 torch.stft 替代 scipy.signal.stft，将 STFT 计算迁移到 GPU。
"""
...
def _stft_gpu(values: np.ndarray, sample_rate: float, window_ms: float, overlap: float, nfft: int | None) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def compute_stft_power(values: np.ndarray, sample_rate: float, window_ms: float, overlap: float, nfft: int | None) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """STFT power spectrum.

    v2: 优先使用 GPU (torch.stft)，回退到 scipy。
    """
    if _torch_available:
        return _stft_gpu(values, sample_rate, window_ms, overlap, nfft)
