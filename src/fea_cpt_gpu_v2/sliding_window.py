"""滑窗批量特征提取模块（v2：ProcessPoolExecutor + GPU STFT + 自动检测核心数）。

用于对连续真实数据（npz/tdms）进行滑窗全量特征提取。
支持多文件夹批量处理、200kHz/500kHz 统一采样率、窗级并行。

v2 优化（对比 v1）：
1. ProcessPoolExecutor 替代 ThreadPoolExecutor，突破 GIL 限制
2. 默认 workers 自动检测 CPU 核心数（保留 2 核给系统）
3. signal_ops.py v2 使用 torch.stft 实现 GPU 加速 STFT
"""
...
class SlidingWindowConfig:
    ...
    window_workers: int | None = None  # None = 自动检测
...
def process_source_file(
    ...
    # v2: ProcessPoolExecutor 替代 ThreadPoolExecutor，突破 GIL 限制
