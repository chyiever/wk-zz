...
## 2026-09-10（补充）图 19 增加 median RSE 与大于 100% 结果解释

- 图 19 的六个面板均同时绘制两类跨特征汇总：虚线为 `median_rse`（典型特征），实线圆点为 `p90_rse`（90% 特征覆盖）；颜色继续区分 BK00/BK05/FL00/FL05/QJ00/QJ05。10% 判稳阈值仍只作用于 P90 RSE。
- RSE 按 `Bootstrap SE / |Bootstrap estimate|` 计算，没有 100% 上限。大于 100% 的物理含义是统计量的抽样标准误大于其绝对估计值，说明该相对估计不稳定，而不是“概率超过 100%”。
- 最新完整运行共有 423,440 个明细项，其中 41,954 项 RSE 大于 100%（9.91%）；仅看各类别最大样本量，共 18,960 项，其中 417 项大于 100%（2.20%）。这表明高 RSE 主要集中在较小样本量区间，增加样本量明显减少了高 RSE 项，但没有消除近零中心量与重尾比值特征带来的问题。
