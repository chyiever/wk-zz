...
-   2.7.2 图中同时展示 BK00、BK05、FL00、FL05、QJ00、QJ05：BK 使用 0-30 ms 单窗口事件样本，FL/QJ 保留原始样本。六类使用相同公式但分别进行类内归一化和 Bootstrap，因此曲线对其他类别的加入保持不变，并可比较相对抽样稳定性。
-   2.7.2 输出 `bk_0_30ms_selection_audit.csv`、BK 专用的 `bk_0_30ms_sample_stability_curve.csv`/`summary.csv`、六类完整的 `bk_0_30ms_with_nonbk_sample_stability_curve.csv`/`summary.csv` 和 `plots/bk_0_30ms_with_nonbk_sample_stability_curves.png`。筛选审计会报告缺失或重复目标窗口；同一事件存在多个目标窗口时直接报错。
