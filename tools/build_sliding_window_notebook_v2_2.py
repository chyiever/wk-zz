"""生成 2026-05-28-realdata_continuous_feature_batch_extract_v2.2.ipynb 笔记本。

v2.2 优化版：持久化进程池 + 共享内存 + GPU 集中化 STFT + 文件级流水线
...
def build_notebook() -> dict:
    ...
    # Cell 0: 标题
    cells.append(make_cell('markdown', '''# 真实连续数据全量特征批量提取（滑窗版 v2.2）

本 notebook 是 v2.1 的**架构级优化版本**。

## v2.2 优化内容（对比 v2.1）

| 优化项 | v2.1 | v2.2 | 预期收益 |
|--------|------|------|----------|
| 进程池 | 每 batch 重建 | **持久化，全程复用** | 消除进程创建/销毁开销，CPU +40% |
| 数据传递 | pickle 全量信号 | **共享内存零拷贝** | 消除 560MB IPC 拷贝 |
| GPU STFT | 各 worker 独立调用 | **主进程集中化批处理** | PCIe 往返 999×2→2，GPU +40% |
| GPU 访问 | 所有 worker 争抢 | **仅主进程** | 消除多进程 GPU 竞争 |
| 文件流水线 | 无 | **后台预加载下一文件** | I/O 与计算重叠，CPU +10% |
...
| 进程生命周期 | 每 batch 重建 | 每 batch 重建 | 每 batch 重建 | **全程复用** |
| 数据传递 | pickle | pickle | pickle | **共享内存零拷贝** |
| STFT 位置 | CPU | GPU (per worker) | GPU (per worker) | **GPU (主进程批处理)** |
...
ENABLE_SHARED_STFT = True      # 跨频带共享 STFT
STFT_BATCH_SIZE = 200          # v2.2: GPU STFT 每批窗口数（控制 GPU 内存）
...
OUTPUT_ROOT = workspace / 'outputs' / 'realdata_continuous_features_20260528_v2_2'
NPZ_PER_CSV = 100
...
print('\\n开始单文件特征计算（v2.2: 持久化进程池 + 共享内存 + GPU集中化STFT）...')
t0 = time.time()
...
print(f'{"="*60}')
print(f'批量处理开始 (v2.2: 持久化进程池 + 共享内存 + GPU集中化STFT + 流水线)')
...
if stats["processed"] > 0:
    ...
    print(f'单文件平均耗时: {avg:.1f} s')
    print(f'v1 基准: 127.0 s, 加速比: {127.0/avg:.2f}x')
print(f'{"="*60}')
'''))
...
'''))
    ...
    # Cell 9: v2.2 架构说明
    cells.append(make_cell('markdown', '''## v2.2 架构说明
...
def main():
    ...
    output_path = Path(__file__).parent.parent / 'notebooks' / '2026-05-28-realdata_continuous_feature_batch_extract_v2.2.ipynb'
    ...
    if replacement_count > 0:
        print('WARNING: replacement characters found')
    else:
        print('Chinese encoding check passed')
